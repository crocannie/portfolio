...
$all = urldecode('index.php?r=site/activities'); 
  ...
  foreach ($cnt as $row) {
    $test = $row['countS'];
          ...
          $sql1 = 'SELECT asp.*, se.name as status, tc.name as typeContest, td.name as typeDocument FROM achievementsSport asp, statusEvent se, eventType tc, typeDocument td WHERE idStudent = :id and asp.status = 0 and asp.idStatus = se.id and asp.idTypeContest = tc.id and asp.idDocumentTYpe = td.id AND asp.year BETWEEN DATE_SUB( NOW( ) , INTERVAL 2 YEAR )AND (curdate( ))';
          $ret1 = Yii::$app->db->createCommand($sql1)
                        ->bindValue(':id', Yii::$app->user->identity->id)
                        ->queryAll();
            $sql2 = 'SELECT * FROM `sportParticipation` WHERE idStudent = :id and sportParticipation.status = 0 AND sportParticipation.date BETWEEN DATE_SUB( NOW( ) , INTERVAL 2 YEAR )AND (curdate( ))';
            $ret2 = Yii::$app->db->createCommand($sql2)
                        ->bindValue(':id', Yii::$app->user->identity->id)
                        ->queryAll();
?>
<?php
      ...
      <tr>
        ...
        <td>    
          ...
          <?php if ($test != 0){ 
              ...
              if ((count($ret1) + count($ret2)) === 0){Yii::$app->session->setFlash('warning', 'Отсутствуют подтвержденные и доступные достижения для анкеты.');}
                else{
      ...
      <tr>
        <td>
          <?php
            ...
            $typeContest = $ret1[$i]['typeContest'];
        ...
        <td>
          <?php
            $name = $ret1[$i]['name'];
            $typeDocument = $ret1[$i]['typeDocument'];
            $year = $ret1[$i]['year'];
...
<?php
        ...
        } elseif ((count($ret1) + count($ret2)) === 0) {
